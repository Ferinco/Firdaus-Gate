            ...
            <div className="body container">
              <div className="headers d-flex flex-row justify-content-between align-items-center">
                ...
                <button className="transfer-btn ">
                  <Link className='react-router-link' to={user.role === "student" ? PATH_DASHBOARD.student.accountSettings : PATH_DASHBOARD.teacher.accountSettings}>Settings</Link>
                </button>
                </div>
              </div>
              <div className="content">
                <div className=" d-flex flex-column mt-4">
                  ...
                  <div className="info d-flex justify-content-between">
                    <p className="text-capitalize w-50">{user.role === "student" ? "admission number:" : "teacher id:"}</p>{" "}
                    <h6 className="text-capitalize w-50">
                      {user.role === "student" ? user?.admissionNumber : user?.teacherId}
                    </h6>
                  </div>
                  <div className="info d-flex justify-content-between">
                    <p className="text-capitalize w-50">{user.role === "student" ? "current class:" : "class handled:"}</p>{" "}
                    <h6 className="text-capitalize w-50">{user.role === "student" ? studentClass : teacherClass}</h6>
                  </div>
               {
                user.role === "teacher" ? (
                  <div className="info d-flex justify-content-between">
                  ...
                  <div className="info d-flex justify-content-between">
                    <p className="text-capitalize w-50">{user.role === "student" ? "parent phone:" : "phone:"}</p>{" "}
                    <h6 className="text-capitalize w-50">
