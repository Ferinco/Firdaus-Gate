...
<div className="divs d-flex flex-row flex-wrap gap-5 mt-5">
    {
      subjects.map((subject, index)=>(
        <Link className={`subject-div d-flex flex-row align-items-center react-router-link ${index % 2 === 0 ? 'even-subject' : 'odd-subject'}`} key={subject._id} to={`${PATH_DASHBOARD.student.scheme}/${subject._id}`}>
    ...
    <div className=" pl-3">
      <h6 className='m-0'>      {subject.name.length > 17 ? `${subject.name.slice(0, 17)}...` : subject.name}
</h6>
    </div>
  </Link>
      ))
